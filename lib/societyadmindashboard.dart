...
class _SocietyAdminDashboardState extends State<SocietyAdminDashboard> {
  ...
  try {
    ...
    final pendingRequests = joinRequests.where((req) => 
      req['status'] == 'pending'
    ).length;
  ...
  return Column(
    ...
    children: [
      ...
      SizedBox(
        ...
        child: Row(
          children: [
            Expanded(child: _buildClickableStatCard(
              'Members', 
        ...
        children: [
          ...
          IconButton(
            ...
            onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => const SocietyAdminPage()),
            ).then((_) => _fetchDashboardData()),
...
Future<List<MeetingNote>> _fetchMeetingNotes() async {
  try {
    ...
    return response.map<MeetingNote>((json) => MeetingNote.fromJson(json)).toList();
  } catch (e) {
...
Widget _buildDesktopQuickActions() {
        ...
        children: [
          Expanded(
            child: _buildActionButton(
              ...
              _navigateToJoinRequests,
              Colors.purple,
              badge: _stats.pendingRequests > 0 ? _stats.pendingRequests.toString() : null,
            ),
          ),
          const SizedBox(width: 8),
          Expanded(
  ...
  Widget _buildActionButton(String label, IconData icon, VoidCallback onPressed, Color color, {String? badge}) {
    return SizedBox(
